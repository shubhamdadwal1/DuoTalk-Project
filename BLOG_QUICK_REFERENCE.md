...
### Get All Blogs
```
GET http://54.206.28.179:3001/api/blogs
Response: [{ _id, title, content, tags, author, likes, createdAt, ... }]
```

### Create Blog
```
POST http://54.206.28.179:3001/api/blogs
Body: {
  title: string,
  content: string,
  tags: [string],
...
### Like Blog
```
POST http://54.206.28.179:3001/api/blogs/:blogId/like
...
```
GET http://54.206.28.179:3001/api/blogs/user/user-firebase-uid
```
...
3. **API URL**: `http://54.206.28.179:3001`
...
```bash
# Get all blogs
curl http://54.206.28.179:3001/api/blogs

# Create blog (needs authorFirebaseUID in body)
curl -X POST http://54.206.28.179:3001/api/blogs \
  -H "Content-Type: application/json" \
  -d '{"title":"Test","content":"Content","tags":["test"],"authorFirebaseUID":"uid"}'

# Like blog
curl -X POST http://54.206.28.179:3001/api/blogs/id/like \
  -H "Content-Type: application/json" \
...
1. **Start Backend**:
   ...
   cd backend
   npm start
   # Server runs on http://54.206.28.179:3001
   ```

...
**Root Cause**: VITE_SOCKET_URL was set to `http://localhost:3001` in docker-compose.yml build args
...
- **Development** (Vite): Backend on `http://localhost:3001`, frontend connects directly
...
### Backend Health
```bash
docker exec duotalk-backend-1 wget -q -O- http://localhost:3001/api/health
# Response: {"status":"Server is running!","database":"Connected"}
```

### API Access
```bash
docker exec duotalk-backend-1 wget -q -O- http://localhost:3001/api/blogs
